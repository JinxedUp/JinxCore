        ...
        Box::pin(async move {
            ...
            if player.gamemode.load() == GameMode::Adventure {
                let msg = branding::brand(
                    TextComponent::text("You are already in Adventure.")
            ...
            let msg = branding::brand(
                TextComponent::text("Gamemode set to Adventure.")
    ...
    fn execute<'a>(
        ...
        Box::pin(async move {
            ...
            for target in targets {
                if target.gamemode.load() == GameMode::Adventure {
                    let msg = branding::brand(
                        TextComponent::text("You are already in Adventure.")
                            .color_named(NamedColor::Yellow),
                    );
                    ...
                    target.set_gamemode(GameMode::Adventure).await;
                    let msg = branding::brand(
                        TextComponent::text("Your gamemode is now Adventure.")
                            .color_named(NamedColor::Green),
                    );
                    target.send_system_message(&msg).await;
                }
            ...
            let msg = if target_count == 1 {
                ...
                branding::brand(
                    TextComponent::text(format!("Set {name} to Adventure."))
                        .color_named(NamedColor::Green),
                )
            } else {
                branding::brand(
                    TextComponent::text(format!("Set {target_count} players to Adventure."))
                        .color_named(NamedColor::Green),
                )
            };
            sender.send_message(msg).await;
...
pub fn gma_command_tree() -> CommandTree {
    CommandTree::new(["gma"], "Set Adventure mode.")
...
pub fn adventure_command_tree() -> CommandTree {
    CommandTree::new(["adventure"], "Set Adventure mode.")
        .then(require(|sender| sender.is_player()).execute(SelfExecutor))
        .then(argument(ARG_TARGET, PlayersArgumentConsumer).execute(TargetExecutor))
}

pub fn a_command_tree() -> CommandTree {
    CommandTree::new(["a"], "Set Adventure mode.")
        .then(require(|sender| sender.is_player()).execute(SelfExecutor))
        .then(argument(ARG_TARGET, PlayersArgumentConsumer).execute(TargetExecutor))
}
