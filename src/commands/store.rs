...
impl CommandExecutor for StoreExecutor {
    fn execute<'a>(
        ...
        Box::pin(async move {
            ...
            match socials.get("store") {
                Some(link) => {
                    let body = TextComponent::text(format!("Store: {link}"))
                        .color_named(NamedColor::Aqua);
                    sender.send_message(branding::brand(body)).await;
                }
                None => {
                    let body = TextComponent::text("Store link is not configured.")
...
pub fn store_command_tree(state: Arc<PluginState>) -> CommandTree {
    CommandTree::new(["store"], "Show the store link.")
        .execute(StoreExecutor { state })
}
